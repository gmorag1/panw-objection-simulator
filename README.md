...
Onboarding reps to a product suite as broad as PANW's takes time, and one of the hardest parts isn't product knowledge - it's developing the instinct to handle pushback in real time. Role-playing with a manager helps, but it's hard to schedule and easy to go easy on people. I wanted something that could be pulled up before a call, used to drill a specific scenario, and give honest feedback without any social softening.
...
Runs entirely in the browser. No backend, no server, no install. The AI plays the customer persona and evaluates the full conversation at the end. 
